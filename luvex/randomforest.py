...
class RandomForest:
    ...
    def fit(self, X, y):
        ...
        for _ in range(self.n_trees):
            ...
            tree.fit_model(X_sample[:, feature_indices], y_sample)
    ...
    def predict(self, X):
        ...
        tree_predictions = np.array([tree.predictions(X[:, features]) for tree, features in self.trees])
    ...
    def predict_proba(self, X):
        ...
        for tree in [tre for tre, _ in self.trees]:
            tree_predictions = tree.predictions(X)  # Each tree predicts a class
